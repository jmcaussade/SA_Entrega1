...
const ReviewForm = ({ fetchReviews, editingReview, clearEditing }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingReview) {
        console.log(`Updating review at URL: http://miapp.localhost/api/reviews/${editingReview._id}`);
        await axios.put(`http://miapp.localhost:5000/api/reviews/${editingReview._id}`, reviewData);
      } else {
        console.log('Creating new review at URL: http://miapp.localhost:5000/api/reviews');
